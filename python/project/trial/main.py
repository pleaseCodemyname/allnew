...
# Secret.json의 API 키
api_key = secrets["jy_youtube_apiKey2"]
...
async def get_combined_graph():
    combined_graph_filename, df = create_combined_dataframe_and_graph()
    return FileResponse(combined_graph_filename, media_type='image/png')

def create_combined_dataframe_and_graph():
    # 폰트 설정
    ...
    client = MongoClient(f"mongodb+srv://{USERNAME}:{PASSWORD}@{HOSTNAME}")
    db = client["project"]
    collections = [db[f"collection{year}"] for year in [2018, 2020, 2022, 2023]]
    ...
    for i, city in enumerate(["Tokyo", "Osaka", "Fukuoka"]):
        ...
        for year in [2018, 2020, 2022, 2023]:
            ...
            if not city_data.empty:
                ...
                # NaN 또는 Inf 값을 가진 막대는 플로팅하지 않습니다.
                if np.all(np.isfinite(y)):
                    bar_width = 0.5  # 막대 그래프의 너비
                    opacity = 0.8  # 막대 그래프의 투명도

                    bars = plt.bar(x, y, bar_width, alpha=opacity, color=year_colors.get(year, "gray"))
        ...
        plt.ylim(ymin=y_min, ymax=y_max)  # 동적 y-축 범위 설정
    ...
    return combined_graph_filename, df