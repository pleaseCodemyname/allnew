...
plt.rc('font', family='Malgun Gothic') #family = 글씨체
...
print('# index 속성을 이용 색인 개체를 확인')
print(member.index)
print('-' * 50)

member.plot(kind='bar', rot=40, ylim=[0, member.max() + 20], use_index=True, grid=False, table=False, color=['r','g','b','y']) #특별한 컬러 지정 color['r'], black은 k임
plt.xlabel('학생 이름')
plt.ylabel('점수')
plt.title('학생별 시험 점수')
